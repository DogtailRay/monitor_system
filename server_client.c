...
void save_packet(packet_t* p)
{
    ...
    u_short             port;
    libnet_t*           libnet_client;
    libnet_ptag_t       udp_client;
    libnet_ptag_t       ipv4_client;
    libnet_ptag_t       eth_client;
    
    
    fwrite(p->data+HEADER_LEN, p->len - HEADER_LEN, 1, file);
    fflush(file);
    
    memset(payload, p->data+HEADER_LEN, p->len - HEADER_LEN);
    payload_s = sizeof(payload);
    port = PORT_MIN + rand() % (PORT_MAX - PORT_MIN);
    
    /* Build UDP header */
    udp_client = libnet_build_udp(
                                     port, /* source port */
                                     LOG_PORT, /* dst port */
                                     ...
                                     libnet_client, /* libnet handle */
                                     udp_client); /* libnet id */
    if(udp_client == -1)
    {
        fprintf(stderr, "Can't build UDP header: %s\n", libnet_geterror(udp_client));
        return 1;
    }
    
    
    src_ip = libnet_get_ipaddr4(libnet_client);
    if (src_ip == 0) {
        perror(libnet_geterror(libnet_client));
        return 1;
    }
    /* Set destination IP */
    ...
    memcpy(&(dst_ip), &(inp->s_addr), sizeof(u_long));
    
    /* Build ipv4 header */
    ipv4_client= libnet_build_ipv4(
                                   LIBNET_IPV4_H + LIBNET_UDP_H + payload_s,/* length */
                                       ...
                                       0, /* payload size */
                                       libnet_client, /* libnet handle */
                                       ipv4_client); /* libnet id */
    if (ipv4_client == -1)
    {
        fprintf(stderr, "Can't build IP header: %s\n", libnet_geterror(libnet_client));
        return 1;
    }
    
    memset(dst_mac, 0x11, MAC_LEN);
    struct libnet_ether_addr* src_hwaddr = libnet_get_hwaddr(libnet_client);
    if (src_hwaddr == NULL) {
        perror(libnet_geterror(libnet_client));
        return 1;
    }
    memcpy(src_mac, src_hwaddr->ether_addr_octet, MAC_LEN);
    eth_client = libnet_build_ethernet(
